...
func main() {
	...
	for i := 0; i < programParams.DownloadThreads; i++ {
		go Download(wg, stopChan, downloadQueue, &downloaded, pClient)
	}
...
func Download(wg *sync.WaitGroup, stopper chan bool, queue *queue.Queue, counter *int32, pClient *client.PremiumizeClient) {
	...
	for {
		...
		case <-ticker.C:
			...
			if item.Item != nil && item.Item.Type == "file" {
				...
				fmt.Println("Downloading: ", dir+item.Item.Name)
